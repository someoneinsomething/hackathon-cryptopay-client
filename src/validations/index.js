...
const SHOP_NAME_EXP = /^[A-z0-9-]{6,16}$/;
const HAS_LETTER_EXP = /.*[a-zA-Z].*/;

export const path = (value = '') => {
  const isValid = SHOP_NAME_EXP.test(value) && HAS_LETTER_EXP.test(value);
  if (!isValid) {
    return getError('VALIDATION.SHOP_NAME');
  }
  return null;
};
